...
from CustomModule.preprocessing.imagetoarraypreprocessor import ImageToArrayPreprocessor
from CustomModule.callbacks.trainingmonitor import TrainingMonitor
#from CustomModule.callbacks.epochcheckpoint import EpochCheckpoint
from CustomModule.io.hdf5datasetgenerator import HDF5DatasetGenerator
from CustomModule.nn.conv.emotionvggnet import EmotionVGGNet
...
ap.add_argument("-s", "--start", type=int, default=0, help="epoch to restart training at")
...
fname = os.path.sep.join([args["checkpoints"], "weights-{epoch:03d}.hdf5"])
checkpoint = ModelCheckpoint(fname, monitor="val_loss", mode="min", save_best_only=True, verbose=1)

callbacks = [checkpoint, TrainingMonitor(figPath, jsonPath=jsonPath, startAt=args["start"])]
...
model.fit_generator(trainGen.generator(), steps_per_epoch=trainGen.numImages // config.BATCH_SIZE,
                    ...
                    epochs=75, max_queue_size=config.BATCH_SIZE * 2, callbacks=callbacks, verbose=1,
                    initial_epoch=args["start"])
